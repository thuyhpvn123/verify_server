...
/***
abigen --abi auth.abi --pkg contract --out auth.go
***/
contract AuthOTP is Initializable, OwnableUpgradeable, UUPSUpgradeable 
{
    ...
    function validateOTP(
        ...
        require(!request.verified || authenticationHashes[wallet] != bytes32(0), "Already verified");

        if (request.method == TypeMethod.Email) {
            require(verificationStates[wallet].phoneVerified,"need to verify phone in advance");
            // ✅ EMAIL
            verificationStates[wallet].emailVerified = true;
            emit EmailVerified(wallet, _identifier);
        } else {
